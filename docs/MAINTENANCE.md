...
**注意 (R7372f60 后按标题分文件夹)**:
- 4 个 R 目录里都建 12 个标题子文件夹 (子文件夹名 = R1 一句话标题, 经 `safe_filename` 过滤 Windows 非法字符)
- R3 末尾会自动整合 R4 完整内容 (用 `## R4 生图提示词（双语）` 区块)
- R1 转录在 `transcriptions/<title>.md` 平铺 (1 文件 1 标题, 不分子文件夹)
- 12 个标题子文件夹 = 每个含 R3.md + R4.md + R6.md = 36 个核心产物 + R2 1~4 个 = 12~48 个

**子文件夹内文件名**:
```
二创短视频文案/<title>/
├── R2-视频号.md            (A 风格, 12 个)
├── R2-小红书.md            (B 风格, 2 个, 可选)
├── R2-抖音.md              (C 风格, 2 个, 可选)
└── R2-公众号.md            (D 风格, 2 个, 可选)
二创通用/<title>/R3.md      (4 版本, 末尾含 R4 整合)
生图提示词/<title>/R4.md    (双语电影质感封面)
字幕/<title>/R6.md          (每行 <=8 字, 无标点)
```
...
├── transcriptions/                    # R1 输出 (12, 平铺 <title>.md)
├── 二创短视频文案/                    # R2 输出 (12 标题子文件夹, 18 文件)
│   └── <title>/
│       ├── R2-视频号.md
│       ├── R2-小红书.md (可选)
│       ├── R2-抖音.md   (可选)
│       └── R2-公众号.md (可选)
├── 二创通用/                          # R3 输出 (12 标题子文件夹, 末尾含 R4 整合)
│   └── <title>/R3.md
├── 生图提示词/                        # R4 输出 (12 标题子文件夹, 独立副本)
│   └── <title>/R4.md
├── 字幕/                              # R6 输出 (12 标题子文件夹)
│   └── <title>/R6.md
