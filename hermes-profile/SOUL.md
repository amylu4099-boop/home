...
### R6. 字幕排版（已采纳 2026-08-11）

**每次二创完成后，自动生成对应的纯文字短视频字幕排版，保存到 `字幕/` 目录。**

- **路径**: `C:\\Users\\Michael\\Desktop\\音视频转录\\字幕\\`
- **文件名**: `YYYYMMDD-字幕＊<一句话标题>.md`
- **触发时机**: 转录完成后自动执行（接 R3 之后）
- **输入**: R3 D 视频脚本（也可以用其他 R 版本生成的二创）
- **提示词**: 见 `scripts/subtitle_format.py` 的 `USER_PROMPT` 常量（用户 2026-08-11 提供）
- **核心约束**:
  - 每行汉字 <= 8 个
  - 无任何标点符号（纯文字 + 数字 + 必要英文/符号如 %）
  - 一句一行
  - 保留原叙事顺序（不调换/重组）
  - 不拆词组（"中华人民共和国"、"互联网" 等）
  - "的" 字不顶行
- **输出格式**: Markdown 文件含一个 ``` ``` ``` 代码块，内部纯文本字幕
- 用户已确认该规则，永久生效。
