...
├── transcriptions/                # R1: 转录结果 (平铺, 文件名=一句话标题)
│   └── <一句话标题>.md
├── 二创短视频文案/                  # R2: 视频号配文二创, 按标题分文件夹
│   └── <一句话标题>/
│       ├── R2-视频号.md            (A 风格)
│       ├── R2-小红书.md            (B 风格, 可选)
│       ├── R2-抖音.md              (C 风格, 可选)
│       └── R2-公众号.md            (D 风格, 可选)
├── 二创通用/                       # R3: 通用二创 (4 版本), 按标题分文件夹
│   └── <一句话标题>/
│       └── R3.md                   (末尾自动整合 R4)
├── 生图提示词/                     # R4: 双语电影质感封面, 按标题分文件夹
│   └── <一句话标题>/
│       └── R4.md
└── 字幕/                           # R6: 字幕排版 (<=8字/无标点), 按标题分文件夹
    └── <一句话标题>/
        └── R6.md
└── scripts/
...
| R2 | 每次转录完成后, 自动生成一份视频号配文风格二创 (默认 A=视频号图书带货, 可选 B=小红书 / C=抖音 / D=公众号), 保存到 `二创短视频文案/<一句话标题>/R2-<风格>.md` (提示词见 `scripts/secondary_create.py`) | 2026-08-10 (2026-08-11 升级多风格) |
| R3 | 每次转录完成后, 再生成一份通用二创 (4 版本: 摘要/文章/短文/脚本), 保存到 `二创通用/<一句话标题>/R3.md` (末尾自动整合 R4, 提示词见 `scripts/general_create.py`) | 2026-08-10 |
| R4 | **双语电影质感短视频封面视觉设计**: 智能判定类型一(主体聚焦/背影不露正脸)或类型二(宏大叙事/无人物), 9:16 竖版 (即梦/可灵/通义万相) + 16:9 cinematic (MJ/DALL-E/SD), 顶部 3/7 区域主副标题 + 暗化遮罩, 电影级光影 (伦勃朗/逆光/丁达尔/冷暖对比), 高级色调 (青橙/黑金/莫兰迪/深邃暗调), 4K/8K 摄影级纹理. 保存到 `生图提示词/<一句话标题>/R4.md` (提示词见 `scripts/image_prompt.py`) | 2026-08-10 (2026-08-11 升级) |
\n| R6 | 二创后自动字幕排版 (每行<=8字/无标点/一句一行/原顺序), 保存到 `字幕/<一句话标题>/R6.md` (提示词见 `scripts/subtitle_format.py`) | 2026-08-11 |\n| R5 | 脚本输出 secret 时自动脱敏（`mask_secret` 前 4 + 后 4）；`.env` 权限收紧到 ACL-only；`scripts/monitor_env.py` 监控 `.env` 文件访问 | 2026-08-10 |
